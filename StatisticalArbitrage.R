...
options(digits = 16)
##Change this to
setwd("C:/Users/llx/Desktop/QuantInsti-Final-Project-Statistical-Arbitrage-master/QuantInsti-Final-Project-Statistical-Arbitrage-master/database/FullList")
########################################################################################
##                                    Functions                                       ##
########################################################################################
...
GenerateRowValue <- function(begin, end, csvData){
  ...
  print(average)
  print(csvData$stdev[end])
  print(csvData$zScore[end])
  return(csvData)
...
GenerateSignal <- function(counter, csvData){
  ...
  if(is.nan(csvData$zScore[counter])==TRUE)
  {
    csvData$signal[counter] <- csvData$signal[counter]
  }
  else if(csvData$adfTest[counter] == 1)
  {
...
BacktestPair <- function(pairData, mean = 35, slippage = -0.0025, adfTest = TRUE, criticalValue = -2.58,
                         startDate = '2004-05-12', endDate = '2015-11-23', generateReport = TRUE){
  # At 150 data points
  ...
  for(i in 1:length(pairData[,2])){
    ...
    if(i > 130){
      ...
      #ADF Test 
      #120 - 90 - 60 
      if(adfTest == FALSE||is.na(adf.test(pairData$spread[(i-60):end], k = 1)[1])==TRUE)
      {
...
#pairData <- PrepareData(data)
#pairdata <- GenerateSignal(120, pairdata)


a <- BacktestPair(data, 35, startDate = '2004-05-12', endDate = '2009-04-10')
